...
the local profile now selects `gpt-6-astra`. A GPT-6 live run has not yet been
recorded, so the earlier result should not be treated as model-specific GPT-6
validation.
